::used to remove blends (blendlayer and addlayer) from a set of specific sequences
...
IF EXIST remove_blend_temp del remove_blend_temp >nul
::remove blends
...
for /f "tokens=*" %%x in (%qc_file%) do (
	...
	IF !sequence_found!==yes (
		set "echo_line=!echo_line:addlayer=////addlayer!"
		echo !echo_line:blendlayer=////blendlayer! )
) >> remove_blend_temp
::replace qc
IF EXIST remove_blend_temp move "remove_blend_temp" "%qc_file%" >nul
